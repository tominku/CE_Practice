...
void r_and_jacobian(vec &r, mat &jac, vec &phi_n, double bias)
{
    ...
    */     
    ...
    {                        
        ...
        jac(i, i+1-offset) = -n_avg1 / dop_avg;
        jac(i, i-offset) = (n_avg1 + n_avg2) / dop_avg;
        jac(i, i-1-offset) = -n_avg2 / dop_avg;
...
void compute_DD_n_from_NP_solution()
{
    ...
    eDensities = eDensities / 1e6;
    std::string n_file_name = fmt::format("DD_highdop_eDensity_{:.2f}.csv", 0.0);
    eDensities.save(n_file_name, csv_ascii);        
