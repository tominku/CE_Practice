...
Region contacts[] = {contact_source, contact_drain, contact_substrate_gnd};
int num_contacts = 3;
...
const string subject_name = "NBN_2D_DD";
...
int main() {    
    ...
    {
        string load_subject_name = "NBN_2D_NP";
        std::string file_name = fmt::format("{}_phi_GB_{:.2f}_DB_{:.2f}.csv", load_subject_name, 0.0, 0.0); 
        cout << file_name << "\n";        
        vec phi_from_NP(N, fill::zeros);
        phi_from_NP.load(file_name);

        file_name = fmt::format("{}_eDensity_GB_{:.2f}_DB_{:.2f}.csv", load_subject_name, 0.0, 0.0); 
        cout << file_name << "\n";        
        vec eDensity_from_NP(N, fill::zeros); 
        eDensity_from_NP.load(file_name);        

        file_name = fmt::format("{}_holeDensity_GB_{:.2f}_DB_{:.2f}.csv", load_subject_name, 0.0, 0.0); 
        cout << file_name << "\n";        
