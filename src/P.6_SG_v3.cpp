...
{        
    ...
    {        
        ...
        if (compute_only_n)
        {
            ...
            c_vector(span(0, N-1)) = (1.0 / ((dop_left + dop_center)/2.0)) * one_vector(span(1, N));
            c_vector(span(N, 2*N-1)) = (1.0 / thermal) * one_vector(span(1, N));        
            // c_vector(span(0, N-1)) = 1.0 * one_vector(span(1, N));
            // c_vector(span(N, 2*N-1)) = 2.0 * one_vector(span(1, N));
            ...
            C.save("c_mat.txt", arma::raw_ascii);
            //mat C = eye(2*N, 2*N);            
            // mat jac_scaled = jac(span(0, 2*N), span(0, 2*N));
            // mat jac_part_scaled = jac(span(N+1, 2*N), span(1, 2*N)) * C;   
            // //printf("jac_part_scaled size: %d %d \n", jac_part_scaled.n_rows, jac_part_scaled.n_cols);
            // jac_scaled(span(N+1, 2*N), span(1, 2*N)) = jac_part_scaled(span(0, N-1), span(0, 2*N-1));
            // jac_scaled = jac_scaled(span(1, 2*N), span(1, 2*N));            
            ...
            //jac_scaled = jac_scaled;
            //vec r_scaled = r(span(1, 2*N));
